...
#define LENGTH_LED 60     // количество светодиодов в ленте
...
void loop() 
{
  ...
  if(parseStart == 1)   //  если прием остановлен
  {
      ...
      if (message < 150) message = 200;                   // защита от выхода руки слева
      if (message > 645) message = 800;                   // защита от выхода руки справа
      message = map(message, 200, 640, 0, 255);           // преобразуем принятые координаты в значения яркости от 0 до 255
